...
def dispatch(operation, data, *, progress=None):
    ...
    if operation == 'state':
        ...
        message += f' {mode}: {result.scan.hashed_files} files hashed; {result.scan.reused_hashes} hashes reused; {result.scan.retries} retries.'
        if not result.scan.complete:
            message = (f'Scan finished with {len(result.scan.issues)} unverifiable path(s). '
                       f'{result.scan.hashed_files} files hashed; {result.scan.reused_hashes} hashes reused. '
                       'Missing-file and move detection deferred. '
                       + ('Verified updates applied; failed records preserved.' if operation == 'apply' else 'Review the scan issues below.'))
